...
Future<bool> firstrun() async {
  ...
  if (prefs.getBool('initialized') ?? false) {
    ...
    try {
      ...
      await db.execute("CREATE TABLE recipe (`Name` varchar(100) PRIMARY KEY,    'text' text, 'time' integer , 'category' text)");
      await db.execute("CREATE TABLE recipeIngredients ('recipe' text,    'ingredient' text, 'quantity' integer )");
      await db.execute("CREATE TABLE recipePics ('recipe' text,    `bytes` blob NOT NULL  )");
      await db.execute("CREATE TABLE recipeCats ('recipe' text,   'cats' text  )");
